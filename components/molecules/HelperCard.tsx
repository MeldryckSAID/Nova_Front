...
export function HelperCard({
  ...
  const statusColors = {
    available: "bg-success-green/10 text-success-green dark:bg-success-green/20",
        ...
        <div className="w-full h-full flex flex-col items-center justify-center space-y-4">
          <div className="relative">
            <div className="w-20 h-20 bg-gradient-to-br from-light-blue-gray to-royal-blue/20 dark:from-blue-gray-dark dark:to-royal-blue/40 rounded-full flex items-center justify-center group-hover:from-royal-blue/20 group-hover:to-royal-blue/40 transition-all duration-300">
              <img src={avatar || "/placeholder.svg"} alt={name} className="w-16 h-16 rounded-full object-cover" />
            </div>
            <div
          ...
          <div className="text-center space-y-2">
            <h3 className="font-semibold text-primary-text dark:text-dark-base-text">{name}</h3>
            <p className="text-sm font-medium text-royal-blue">{title}</p>
            {specialty && <p className="text-sm text-primary-text/70 dark:text-dark-base-text/70">{specialty}</p>}
            <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${statusColors[status]}`}>
              {status === "available" ? "Disponible" : status === "busy" ? "Occupé" : "Hors ligne"}
            </span>
          </div>
        </div>
      </div>
