...
export default function DashboardPage() {
  ...
  const contactedHelpers = helpers.filter((helper) => user.contactedHelpers?.includes(helper.id))
          ...
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
              <Card className="shadow-lg bg-white dark:bg-blue-gray-dark border-light-blue-gray/20 dark:border-royal-blue/30">
                <CardHeader>
                  <CardTitle className="text-primary-text dark:text-dark-base-text">Vos besoins</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-primary-text/70 dark:text-dark-base-text/70">
                    {user.needs || "Aucun besoin spécifique renseigné pour le moment."}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="shadow-lg bg-white dark:bg-blue-gray-dark border-light-blue-gray/20 dark:border-royal-blue/30">
                <CardHeader>
                  <CardTitle className="text-primary-text dark:text-dark-base-text">Vos domaines d'intérêt</CardTitle>
                </CardHeader>
                <CardContent>
                  {user.specialties && user.specialties.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {user.specialties.map((specialty: string, index: number) => (
                        <Badge key={index} className="bg-royal-blue text-white hover:bg-royal-blue/90">
                          {specialty}
                        </Badge>
                      ))}
                    </div>
                  ) : (
                    <p className="text-primary-text/70 dark:text-dark-base-text/70">Aucune spécialité renseignée.</p>
                  )}
                </CardContent>
              </Card>
            </div>
            ...
            <Card className="shadow-lg bg-white dark:bg-blue-gray-dark border-light-blue-gray/20 dark:border-royal-blue/30">
              <CardHeader>
                <CardTitle className="text-3xl text-primary-text dark:text-dark-base-text">Mes Réservations</CardTitle>
              </CardHeader>
              <CardContent>
                {(() => {
                  // Récupérer les réservations de l'utilisateur
                  const bookings = JSON.parse(localStorage.getItem("bookingRequests") || "[]")
                  const userBookings = bookings.filter((booking: any) => booking.studentId === user.id)
                          ...
                          >
                            <CardContent className="p-6">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-4">
                                  {helper && <Avatar src={helper.avatar} alt={helper.name} size="md" />}
                                  <div>
